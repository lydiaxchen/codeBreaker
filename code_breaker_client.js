 ...
 * 1 line to display the code images - "coderow"
 * 8 attempts
 * 1 accept button
 * 8 peg selections
 */
function createGameBoard(){
    ...
    for (var i = NUM_ATTEMPTS; i > 0; i--){
		...
		var newSpanResu = document.createElement("span");
		$(newSpanResu).attr("id", "attempt"+i+"result");
		$(newDiv).append(newSpanResu);
...
function response(data, status){
    ...
    if (response['action'] == 'generateCode'){
        ...
        if (current_attempt_id < NUM_ATTEMPTS && !win){
            ...
            win? alert("You win! Click enter to play again.") // won!!!
